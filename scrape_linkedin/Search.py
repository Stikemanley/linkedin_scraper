...
class Search(object):
    ...
    def get_links(self):
        links = []
        industries = []
        results = self.soup.findAll('div',{'class':'search-result__wrapper'})

        print(len(results))
        for result in results:
            links.append(result.find('a')['href'])
            industries.append(result.find('p',{'class':'subline-level-1'}).text.strip())

        return links,industries
